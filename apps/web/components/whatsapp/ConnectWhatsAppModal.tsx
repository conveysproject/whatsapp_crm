...
const STEP_LABELS: Record<Step, string> = {
  choose: "Step 1 of 2 — Number Type",
  verify: "Step 2 of 2 — Verify & Connect",
};
    ...
    <div className={variant === "modal" ? `bg-white rounded-2xl shadow-2xl w-full ${isWide ? "max-w-3xl" : "max-w-xl"}` : "w-full"}>
        ...
        <div className="px-6 pt-4 pb-0 flex items-center gap-3">
          ...
          <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: step === "choose" ? "50%" : "100%" }}
            />
          </div>
        </div>
      )}
      ...
      <div className={`flex items-center justify-between ${variant === "modal" ? "px-6 pt-4 pb-4 border-b" : "pb-3 border-b"}`}>
        <h2 className="text-lg font-semibold text-gray-900">
          {step === "choose" ? "2 Ways to Setup WhatsApp API Number" : "Verify Your Business"}
        </h2>
...
function VerifyStep({ flow, isSMB, errorMsg, onBack, onSuccess, onError }: VerifyStepProps): JSX.Element {
  ...
  function handleFile(e: ChangeEvent<HTMLInputElement>): void {
    ...
    fetch("/api/v1/whatsapp-account/verification-upload", { method: "POST", body: formData })
      .then((r) =>
        r.json().then((json: { data?: { url: string }; error?: { message?: string } }) => {
          if (!r.ok) {
  ...
  return (
    <div className="px-6 py-5 space-y-5">
      ...
      <div className={alreadyVerified ? "opacity-40 pointer-events-none select-none" : ""}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="border border-green-200 rounded-xl p-4 space-y-3">
            ...
            {uploading ? (
              <div className="border border-gray-200 bg-gray-50 rounded-lg px-3 py-2.5 flex items-center gap-2 text-sm text-gray-500">
                <span className="w-4 h-4 border-2 border-gray-400 border-t-transparent rounded-full animate-spin shrink-0" aria-hidden="true" />
                Uploading…
              </div>
            ) : gstFile && uploaded ? (
              <div className="border border-green-400 bg-green-50 rounded-lg px-3 py-2.5 space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => { setGstFile(null); setUploaded(false); if (fileInputRef.current) fileInputRef.current.value = ""; }}
              ...
              <div className="border border-red-200 bg-red-50 rounded-lg px-3 py-2.5 space-y-1.5">
                <p className="text-xs text-red-600">{uploadError}</p>
                <button
                  type="button"
                  onClick={() => { setGstFile(null); setUploadError(""); if (fileInputRef.current) fileInputRef.current.value = ""; }}
      ...
      {/* Footer — connect directly without a separate Step 3 */}
      <div className="pt-2 border-t border-gray-100 space-y-2">
        {!alreadyVerified && (
          <p className="text-xs text-center text-gray-400">Verification is optional — you can connect first and verify later</p>
        )}
        <EmbeddedSignupButton flow={flow} isSMB={isSMB} onSuccess={onSuccess} onError={onError} />
      </div>
    </div>
  );
}
