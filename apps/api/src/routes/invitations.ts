  ...
  fastify.post<{ Body: { email: string; role: Role } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        fastify.log.error({ err, to: invitation.email }, "Failed to send invitation email");
        return reply.status(500).send({ error: { code: "EMAIL_FAILED", message: "Invitation created but email could not be sent. Check SMTP configuration." } });
      }
